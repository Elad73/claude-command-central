...
Last updated: 2026-06-28 (fixed K-PIT-004: ghost agents from un-closed sessions)
...
- **Fixed ghost agents from un-closed sessions** *(`K-PIT-004`, 2026-06-28, branch `fix/stale-active-agents`)* — a stale `active` robot kept showing for projects whose session was off (live repro: `personal-space/taxonomy-curator`). Root cause: subagents only clear via `SubagentStop`, which Claude Code sometimes drops; the `Stop` hook marks only the main agent done, and the despawn GC reaped only done/idle, so orphaned active agents lingered forever. Fix: (1) session-end cascade — `flow → done` evicts that project's still-active/blocked agents in both reducers (`evictWorkingAgents`); (2) crash backstop — `pruneStale` + client GC reap active/blocked after `ACTIVE_STALE_TTL_MS` (30 min). Tests 136/136, typecheck clean. See `docs/SCENE-DEFECTS.md`.
